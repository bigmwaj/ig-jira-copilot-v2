...
public class UserStoryRefinementRoute extends BaseOrchestrationRoute {
    ...
    public void configure() {
        ...
        from("timer:refinementPoller?period={{app.routes.refinement-schedule}}")
                ...
                .process(exchange -> {
                    var jql = jiraProcessor.buildJqlQuery(AiState.AI01_WAITING_REFINEMENT);
                ...
                .log(LoggingLevel.INFO, log, "Route 1: Processing issue ${body} for refinement")
        ...
        from("seda:refinement-process?concurrentConsumers=5")
                ...
                .process(exchange -> {
                    ...
                    Map<String, Object> payload = jiraProcessor.buildAiStateUpdatePayload(AiState.AI02_REFINEMENT_IN_PROGRESS);
                ...
                .log(LoggingLevel.INFO, log, "Route 1: AI Status Tracking Change payload: ${body}")
                ...
                .log(LoggingLevel.INFO, log, "The copilot request is ${body}")
                ...
                .process(exchange -> {
                    ...
                    Map<String, Object> payload = jiraProcessor.buildDescriptionAndStateUpdatePayload(
                            aiContent, AiState.AI03_REFINEMENT_COMPLETED
                    );
                    jiraProcessor.setJiraHeaders(exchange, issueKey, HttpMethods.PUT.name());
                    exchange.getIn().setBody(objectMapper.writeValueAsString(payload));
                })
                .log(LoggingLevel.INFO, log, "Route 1: Jira update payload with AI response: ${body}")
                .toD("{{camel.http.jira-issue-uri}}/${header.JiraIssueKey}?throwExceptionOnFailure=false")
